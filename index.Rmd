...
### Tempogram 1 - "Trip" by Ella Mai

```{r}
TripElla <- get_tidy_audio_analysis("6CTWathupIiDs7U4InHnDA")
```
...
Using the same song (Trip by Ella Mai), I created a tempogram. The clearest yellow line shows around the 320 BPM, which is four times the original BPM of the song (BPM). We can also see some tempo harmonics around 
In between 


### Tempogram 2 - "Lost" by Frank Ocean
```{r}
Lost <- get_tidy_audio_analysis("3GZD6HmiNUhxXYf8Gch723") # by Frank Ocean
```
...
The song "Lost" by Frank Ocean is an example where there is one very strong and clear line representing tempo at around 240 BPM This is very interesting because the actual BPM of the song is 123 BPM The tempogram clearly shows the tempo harmonics, in this case, double time of 246 BPM There is a faint line below the main 246 line, it represents the actual BPM of the song. Additionally, we also see another tempo harmonic above at around 500 BPM, which is four times the original BPM (496 BPM). 
